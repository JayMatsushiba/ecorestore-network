...
| **Python acquisition writes the committed fixture** | **Rejected** | It would make Python a second canonicaliser. The job writes an unhashed snapshot; `npm run acquire:finalize` attaches `geometryHash`, `h3Root` and `snapshotHash` and compares against the current fixture before writing. The job also emits the geometry and H3 resolution it read, and finalize refuses to attach an identity that geometry does not hash to — a matching `parcelId` is not a matching parcel. |
| **Open-ended dependency ranges for the analysis image** | **Rejected** (2026-09-10) | Bit-exact parity is the acceptance test and floats belong to the build that produced them, so a rebuild resolving a newer NumPy could return different numbers under an unchanged `analysisEngine`. `analysis/constraints.txt` pins the resolved set; moving a pin means re-running parity in the image and bumping the engine version. |
| **Timing in the analysis response body** | **Rejected** (2026-09-10) | The caller assembles a hashed document from that response. The body is the contract's `AnalysisOutput` and nothing else; elapsed time is a header. |
| **`sent` as evidence of Guardian acceptance** | **Rejected** (2026-09-10) | The adapter reports any HTTP answer as `sent`. Only a 2xx is rendered as acknowledgement; a 4xx or 5xx is rendered as rejected. The distinction is the same one §2 draws between acknowledgement and a policy run, one level down. |
| **Committed reference files as the parity baseline on their own** | **Rejected** (2026-09-10) | They are dumps, so nothing tied them to the live TypeScript engine: a change to `analyseTier0()` would leave the Python suite green against stale files. A vitest case recomputes all five and fails on drift. |
...
| **Basemap tiles and client-side raster on the web map** | **Rejected** (2026-09-10) | The map draws vectors only: parcel, rings, matched control cells, the pixel window and the simulated evidence points, over its own graticule and scale bar. No basemap, so the demonstration never depends on a third party's tile service and the recorded video renders offline; a link opens the location in OpenStreetMap instead. No client-side NDVI: reading the COGs in the browser would compute the index a second time outside the processing graph the result cites. A server-rendered raster from the analysis engine is the follow-on if one is wanted. |
| **Geometry inside `VerificationResult` for the map** | **Rejected** (2026-09-10) | `resultHash` is over the result and the deed cites it. The map's geometry travels in a `spatial` block on the bundle envelope, produced by the same helpers the engine uses, and a test pins the hash unchanged. |
| **Generated build-state table on the About page** | **Deferred** (2026-09-10) | The page states build state in one hand-written table that mirrors `ARCHITECTURE.md` §8, so a single edit keeps it true. A machine-readable component list does not exist yet, and inventing one for a copy page is the tail wagging the dog. |
...
| **Real Tier 0 acquisition** | **Resolved for the current site.** The parcel is in the interior Creston Valley (−116.58, 49.13), not coastal BC, and 72 Sentinel-2 scenes were acquired across three seasons, 61 of them usable over the parcel after SCL masking. The general rule stands for any new site: pick the parcel and window from actual archive availability, not the reverse. |
