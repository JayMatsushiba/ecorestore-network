...
**No production deployment has occurred.** A continuous-deployment pipeline exists (`docs/AWS_DEPLOYMENT.md`, `deploy/README.md`): every push to `main` builds `docker-compose.yml`'s stack into two images and restarts it on one EC2 demonstration host. What it hosts is still the local demo — in-memory Hardhat chain, Docker Graph Node, MockUSDC, MockGuardianAdapter, synthetic fixtures — on a public URL. No hosted/decentralized Graph Network, no live Hedera Guardian, no Arc Testnet/Mainnet deployment, no real USDC, no real funds.
